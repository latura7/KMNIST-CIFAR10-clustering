...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
        ...
        \midrule
        ResNet18 supervised full, 20 epochs & 0.6408 & 0.7252 & 0.7825 & 0.2747\\
        ResNet18 supervised full, 100 epochs & 0.7177 & 0.7571 & 0.8648 & \textbf{0.3616}\\
...
ResNet18 full run 은 CIFAR-10 label 로 100 epochs 까지 학습하면 20 epochs 보다 좋아졌지만, DINOv2 frozen feature 보다는 ARI/NMI/purity 가 낮았다.
...
\begin{figure}[H]
    ...
    \caption{CIFAR-10 optional DINOv2 feature comparison. DINOv2 improves label-based clustering metrics over the supervised ResNet18 full runs, while ResNet18 gives higher silhouette.}
...
\subsection{CIFAR-10 ResNet18 Epoch Sweep}

CIFAR-10 에서는 supervised ResNet18 feature 가 20 epochs 만으로 완전히 수렴했다고 보기 어려웠다.
따라서 full train split 을 유지한 채 학습 epoch 만 20, 50, 100 으로 늘려 추가 실험을 수행하였다.
Clustering 은 동일하게 StandardScaler 와 K-means \(k=10\)을 사용하였다.

\begin{table}[H]
    \centering
    \caption{CIFAR-10 ResNet18 full-split epoch sweep. Clustering metrics are measured on the test split.}
    \label{tab:cifar-resnet-epoch-sweep}
    \begin{tabular}{cccccc}
        \toprule
        Epochs & ARI & NMI & Purity & Silhouette & Classifier eval acc\\
        \midrule
        20 & 0.6408 & 0.7252 & 0.7825 & 0.2747 & 0.8636\\
        50 & 0.6920 & 0.7467 & 0.8478 & 0.3492 & 0.8672\\
        100 & \textbf{0.7177} & \textbf{0.7571} & \textbf{0.8648} & \textbf{0.3616} & 0.8344\\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{resnet_epoch_sweep_clustering_metrics.png}
        \caption{Clustering metrics}
    \end{subfigure}
    \begin{subfigure}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{resnet_epoch_sweep_classifier_summary.png}
        \caption{Classifier summary}
    \end{subfigure}
    \caption{CIFAR-10 ResNet18 epoch sweep on the full train split. Longer training improves the clustering feature, although classifier evaluation accuracy does not increase monotonically.}
    \label{fig:cifar-resnet-epoch-sweep}
\end{figure}

Table~\ref{tab:cifar-resnet-epoch-sweep}와 Figure~\ref{fig:cifar-resnet-epoch-sweep}에서 ARI 는 20 epochs 의 0.6408에서 100 epochs 의 0.7177까지 상승하였다.
Purity 도 0.7825에서 0.8648로 좋아졌다.
이는 CIFAR-10 에서 ResNet18 feature 의 성능이 학습 시간에 영향을 받는다는 점을 보여준다.
다만 classifier eval accuracy 는 100 epochs 에서 0.8344로 낮아졌으므로, classification accuracy 와 clustering feature quality 가 항상 같은 방향으로 움직인다고 보기는 어렵다.
Longer training 자체는 도움이 되었지만, 더 안정적인 개선을 위해서는 learning rate scheduler 나 regularization 설정까지 함께 조정할 필요가 있다.
...
CIFAR-10 에서는 main 20-epoch setting 에서 ARI 0.6408, NMI 0.7252, purity 0.7825 를 얻었다.
추가 epoch sweep 에서는 100 epochs 학습 시 ARI 0.7177, NMI 0.7571, purity 0.8648 까지 상승하였다.
...
그 결과 DINOv2 + K-means 는 ARI 0.7969, NMI 0.8414, purity 0.9053 을 얻어 CIFAR-10 ResNet18 100-epoch result 보다도 높은 label-based clustering metric 을 보였다.
이 결과는 natural image clustering 에서 대규모 pretrained visual representation 이 강력한 baseline 이 될 수 있음을 보여준다.

한계도 있다.
ResNet18 epoch sweep 에서 100 epochs 까지 성능이 상승했으므로, CIFAR-10 feature extractor 학습에는 아직 tuning 여지가 남아 있다.
다만 classifier eval accuracy 는 100 epochs 에서 단조 증가하지 않았기 때문에, 단순히 epoch 만 늘리는 것보다 learning rate scheduler, regularization, contrastive pretraining, self-labeling 같은 방법을 함께 조정하는 것이 더 타당하다.
...
CIFAR-10 ResNet18 50/100 epoch sweep rows are also included in \texttt{experiments.csv}.
...
\begin{lstlisting}
python generate_sweep_plots.py
\end{lstlisting}

최신 batch 비교 결과 요약은 \texttt{final\_compare\_summary}에 포함하였다.
이 폴더에는 주요 비교 CSV 와 metric bar plot 만 저장하였다.
